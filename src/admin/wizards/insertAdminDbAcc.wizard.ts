...
export class InsertAdminDbAccWizard {
  ...
  async enterNameAccount(@Ctx() ctx: Scenes.WizardContext) {
    ...
    await ctx.reply('🧑‍💻 <i>Write username of admin</i>', {
      parse_mode: 'HTML',
    });
    ctx.wizard.next();
  }
  ...
  async enterUserId(@Ctx() ctx: Scenes.WizardContext) {
    if (!ctx.message) {
      await ctx.reply(
        '❌ <i>ERROR: please write correct username. Repeat action "Insert admin in database" to correct insert</i>',
      );
    }
  ...
  async insertAdminToDb(@Ctx() ctx: Scenes.WizardContext) {
    ...
    await this.adminService.insertAdminDbAcc(
      ctx,
      true,
      this.username,
      this.userId,
